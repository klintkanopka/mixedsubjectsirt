...
item_pars_from_vector_1pl <- function(par, item_names) {
  ...
  if (length(par) != n_items + 1L) {
    stop(
      "1PL par must have length n_items + 1 (one shared a, then d_1 ... d_J).",
      call. = FALSE
    )
  }
  out <- data.frame(
...
fit_from_counts_1pl <- function(
  ...
  objective <- function(par) {
    ...
    val <- loss_expected_counts(counts_observed, ip) +
      lambda *
        (loss_expected_counts(counts_generated, ip) -
  ...
  gradient <- function(par) {
    if (!is.finite(par[1]) || par[1] <= 0) {
      return(rep(0, n_items + 1L))
    }
    ip <- item_pars_from_vector_1pl(par, item_names)
    g <- gradient_expected_counts_1pl(counts_observed, ip) +
      lambda *
        (gradient_expected_counts_1pl(counts_generated, ip) -
...
#' )
#' fit$item_pars
fit_mixed_subjects_1pl <- function(
  observed,
  predicted,
  generated,
  lambda = 1,
...
#' )
#' fit$item_pars
fit_mixed_subjects_mml_1pl <- function(
  observed,
  predicted,
  generated,
  lambda = 1,
...
print.mixedsubjects_1pl_fit <- function(x, ...) {
  ...
  if (isTRUE(x$mml)) {
    cat("  estimator:  marginal MML PPI++ (1PL)\n")
  }
  invisible(x)
}
