...
`tune_lambda_ability` to select $\lambda = 0$ — correctly recovering the human-only
...
$\lambda = 0.5$) arise when the linked parameters destabilise the optimizer before
`tryCatch` can catch them at the sweep stage; the underlying cause is the same
gradient asymmetry at extreme linking constants. `tune_lambda_ability` correctly
