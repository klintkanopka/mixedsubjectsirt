...
# Discriminations are clamped to slope_lower (1e-4) so that mirt fits with
# near-zero item parameters on different platforms do not destabilise the
# downstream expected-count optimisation.
...
- **`tune_lambda_ppi_score()`** returns the PPI++ Proposition 2 estimate: the $\lambda$
  that minimises the *trace of the item-parameter covariance matrix* $\text{Tr}(\Sigma_\gamma)$.
  This is a measure of item-parameter estimation efficiency.
- **`tune_lambda_ability()`** returns the $\lambda$ that minimises the *propagated
...
practice, users should select $\lambda$ by ability risk (`tune_lambda_ability`), not by
...
knitr::kable(val_tab, row.names = FALSE,
  ...
  caption   = paste("PPI++ score lambda vs. ability-risk lambda.",
                    "PPI++ lambda minimises Tr(Sigma_gamma).",
                    "Ability-risk lambda minimises E[g' Sigma_gamma g]."))
```
...
The ability-risk criterion (`tune_lambda_ability`) selects $\lambda$ based on scoring
...
psychometric applications, always use `tune_lambda_ability()` to select $\lambda$.** The
