...
Baseline models are estimated by marginal maximum likelihood with `fit_2pl()`, using the same objective and quadrature as the mixed-subjects estimator below.
...
`vcov()` on a scalar-lambda MML fit automatically uses `vcov_mixed_subjects_mml()`, which applies Louis' observed-information correction.[^3] Here, the bread is $H_\mathrm{comp} - I_\mathrm{miss}$ rather than the EM complete-data Hessian alone.
...
[^1]: [Liu, C. W., & Chalmers, R. P. (2021). A note on computing Louis’ observed information matrix identity for IRT and cognitive diagnostic models. _British Journal of Mathematical and Statistical Psychology_, 74(1), 118-138.](https://doi.org/10.1111/bmsp.12207)
[^2]: [Mani, P., Xu, P., Lipton, Z. C., & Oberst, M. (2025). No free lunch: Non-asymptotic analysis of prediction-powered inference. _arXiv preprint arXiv:2505.20178_.](https://arxiv.org/abs/2505.20178)
[^3]: [Louis, T. A. (1982). Finding the observed information matrix when using the EM algorithm. _Journal of the Royal Statistical Society Series B: Statistical Methodology_, 44(2), 226-233.](https://doi.org/10.1111/j.2517-6161.1982.tb01203.x)